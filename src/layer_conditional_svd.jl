export ConditionalSVDLayer

using InvertibleNetworks: InvertibleNetworks, NeuralNetLayer, Parameter
using LinearAlgebra: svd
import Flux

mutable struct ConditionalSVDLayer <: NeuralNetLayer
    Ux::Parameter
    Vtx::Parameter
    Sx::Parameter
    Uy::Parameter
...
@Flux.functor ConditionalSVDLayer

function ConditionalSVDLayer(; logdet=false)
    s = Parameter(nothing)
    b = Parameter(nothing)
    ...
    return ConditionalSVDLayer(Ux, Vtx, Sx, Uy, Vty, Sy, logdet, false)
end

function initialize!(LN::ConditionalSVDLayer, X::AbstractArray{T, Nx}, Y::AbstractArray{T, Ny}) where {T, Nx, Ny}
...
function InvertibleNetworks.forward(X::AbstractArray{T, Nx}, Y::AbstractArray{T, Ny}, LN::ConditionalSVDLayer; logdet=nothing) where {T, Nx, Ny}
...
function InvertibleNetworks.inverse(Z::AbstractArray{T, N}, Y::AbstractArray{T, N}, LN::ConditionalSVDLayer) where {T, N}
    bias = compute_bias(LN, Y)
    X = Z .+ reshape(bias, size(Z))
    return X
end

function InvertibleNetworks.backward(ΔZ::AbstractArray{T, N}, Z::AbstractArray{T, N}, Y::AbstractArray{T, N}, LN::ConditionalSVDLayer; set_grad::Bool = true) where {T, N}
